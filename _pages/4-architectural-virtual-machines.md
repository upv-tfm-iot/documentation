...
```mermaid
architecture-beta
    group m1(logos:apple)[Macbook Pro M1  VMWare Fusion]
    group k8s[Kubernetes] in m1
    group support(logos:debian)[Support] in m1
    
    group controlplane[Kubernetes Control Plane] in k8s
    group vmcontrolplane(logos:debian)[VM Control Plane] in controlplane

    group workers[Kubernetes Workers] in k8s

    group vmworker1(logos:debian)[VM Worker 1] in workers
    group vmworker2(logos:debian)[VM Worker 2] in workers
